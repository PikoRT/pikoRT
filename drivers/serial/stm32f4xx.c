...
int stm32f4_getc(struct serial_info *serial, char *c)
{
    serial->rx_count--;
    cbuf_getc(&cbuf, c);
...
int stm32f4_putc(struct serial_info *serial, char c)
{
...
int stm32f4_puts(struct serial_info *serial,
                   size_t len,
                   size_t *retlen,
                   const char *buf)
{
...
struct serial_info stm32f4_uart1 = {
